...
data class Attack(val name: String = "", val verb: String, val damage: Double, val cooldown: UByte = 0u)
...
class Sequencer(private val app: App) {
    fun combat(enemy: Enemy) {
        ...
        while (true) {
            ...
            if (app.health == 0.0) {
                app.displayTextAndReRender("Tragically, you have passed away due to your wounds in battle. Unfortunately, you do not magically come back to life in this world, so this is goodbye.")
                sleep(1.0)
                exitProcess(0)
            }
